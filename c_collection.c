...
bool init_conf(void)
{
	char *conf_json = NULL;
	FILE *conf_fd = NULL;
	if (access("/home/cf/conf/cCollection.conf.json", F_OK) != -1)
	{
		if (access("/home/cf/conf/cCollection.conf.json", R_OK) != 0)
		{
	...
	else
	{
		...
		printf("conf:%s\n", conf_json);
		if ((conf_fd = fopen("/home/cf/conf/cCollection.conf.json", "a+")) == NULL)
		{
